...
const char* CHILD = "cubicantipodal-rayper2julia.so";
...
int iter(Point z, Parameter param, int max)
{
  ...
  if (i<max) {
    if (norm(x) < eps) {
      x *= -q;
      return (3*i + 51*((slope*real(x)>imag(x)) && (slope*real(x) > -imag(x)))) % 255;
    } else {
      x /= q;
      return (3*i+1+ 51*((slope*real(x)>imag(x)) && (slope*real(x) > -imag(x)))) % 255;
    }
  } else
