...
const char* NAME = "Cubic parabolic map with Lavaurs Map (parameter space)";
const char* CHILD = "cubiclavaursjulia2.so";
...
{
    complex<double> a(param.Value(0),param.Value(1)), A=1.0/(a*a), x;
    register int i;
    complex<double> pha(z.x,z.y);
    
    x = (-a+sqrt(a*a-3.0))/3.0;
    for ( i=0; (i<max) && (norm(x)<16); i++ ) {
	x=f(x, a, param.Value(2), pha);
    }
...
Point map(Point z, Point c, Parameter param)
{
	...
	a(param.Value(0),param.Value(1)), A=1.0/(a*a);
    complex<double> pha(z.x,z.y);
    x=f(x, a, param.Value(2), pha);
