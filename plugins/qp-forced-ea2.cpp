...
const char* CHILD = "qp-forced-eajulia.so";
...
const int N=4;
double VALUE[N]={ 0.42, 0, 0, (sqrt(5)-1)/2 };
const Parameter PARAM( N, VALUE );
const char * PARAMDESC[N] = { "re(eps)", "im(eps)", "theta", "omega" };
...
int iter(Point z, Parameter param, int max)
{
  ...
  Complex eps(param.Value(0),param.Value(1));
  Complex a(z.x,z.y); 
...
{
  double omega(param.Value(3));
  Complex a(c.x,c.y), eps(param.Value(0),param.Value(1));
  Complex x(z.x, z.y);
