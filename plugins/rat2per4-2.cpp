...
const char* CHILD = "rat2per4-2julia.so";
...
int iter(Point z, Parameter param, int max)
{
  ...
  a = - ((sqrt(5)-1)*a+sqrt(5)+1.0) / (2.0*(a-1.0));
  complex<double> n=a*(a-1.0), p=a-2.0, q=1.0-a, 
...
Point map(Point z, Point c, Parameter param)
{
  ...
  a = - ((sqrt(5)-1)*a+sqrt(5)+1.0) / (2.0*(a-1.0));
  complex<double> n=a*(a-1.0), p=a-2.0, q=1.0-a, x(z.x,z.y);
  FUNCTION
...
Point init(Point c, Parameter param)
{
  complex<double> a(c.x, c.y);
  a = - ((sqrt(5)-1)*a+sqrt(5)+1.0) / (2.0*(a-1.0));
  complex<double> x= (a*(a-1.0)-1.0)/(a-2.0)/2.0;
  return Point(real(x),imag(x));
}  
