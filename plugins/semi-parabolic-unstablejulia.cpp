 ...
 * unstable slice for period 2 saddle (phase space)
 */
...
const char* NAME = "Semi-parabolic implosion (unstable slice)";
...
void paraminit(Parameter param)
{
  complex<double> e(param.Value(0),param.Value(1)),
    a(param.Value(2),param.Value(3)), f=e*e/(1.0-a);
...
{
  complex<double> e(param.Value(0),param.Value(1)),
    a(param.Value(2),param.Value(3)), x(z.x,z.y), y, tmp;
