/*
 * 6-modal Blashke product (Julia set, with rays)
 */
...
const char* NAME = "6-modal Blaschke product (Julia set, with rays)";
...
void paraminit(Parameter param)
{
  ...
  p = lambda * power(-r,k1) * power(-b2,k2) * power(-b3,k3);
  p = pow(p,1.0/(k-1));
